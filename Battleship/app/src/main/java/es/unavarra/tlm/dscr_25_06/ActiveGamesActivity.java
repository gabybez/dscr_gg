...
public class ActiveGamesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle b) {
        ...
        adapter = new GameListAdapter(this, new GameListAdapter.Actions() {
            @Override public void onAccept(GameInfo g) { }
            @Override public void onReject(GameInfo g) { }
            @Override public void onCancel(GameInfo g) { }
        });
    ...
    private void realizarCarga(boolean mostrarSpinner) {
        ...
        client.get(url, new ListarActivosResponseHandler(this, games -> {
            ...
            // Filtrar: solo placing y started (excluir waiting)
            allGames.clear();
            if (games != null) {
    ...
    private void filtrar(String query) {
        ...
        if (filtered.isEmpty()) {
            ...
            tvEmpty.setText(q.isEmpty()
                    ? getString(R.string.list_empty_active)
